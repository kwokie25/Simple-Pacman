...
public class GameDisplay extends Application {
    ...
    // audio clips used in game
    AudioClip introMusic = new AudioClip(getClass().getClassLoader().getResource("sounds/pacman_beginning.wav").toExternalForm());
    // Media intro = new Media ( getHostServices().getDocumentBase() + "build/resources/main/sounds/pacman_beginning.wav");
    // MediaPlayer introMusic = new MediaPlayer(intro);
    AudioClip endingSound = new AudioClip(getClass().getClassLoader().getResource("sounds/pacman_death.wav").toExternalForm());
    // Media end = new Media (getHostServices().getDocumentBase() + "build/resources/main/sounds/pacman_death.wav");
    // MediaPlayer endingSound = new MediaPlayer(end);
    AudioClip munchSound = new AudioClip(getClass().getClassLoader().getResource("sounds/pacman_chomp.wav").toExternalForm());
    // Media munch = new Media (getHostServices().getDocumentBase() + "build/resources/main/sounds/pacman_chomp.wav");
    // MediaPlayer munchSound = new MediaPlayer( munch );
    ...
    public GameDisplay() {
        ...
        for (int i = 0; i < 3; i++) {
            upPacman[i] = new Image( getClass().getClassLoader().getResource("images/pacUp" + i + ".png").toString());
        }

        for (int i = 0; i < 3; i++) {
            downPacman[i] = new Image(getClass().getClassLoader().getResource("images/pacDown" + i + ".png").toString());
        }

        for (int i = 0; i < 3; i++) {
            leftPacman[i] = new Image(getClass().getClassLoader().getResource("images/pacLeft" + i + ".png").toString());
        }

        for (int i = 0; i < 3; i++) {
            rightPacman[i] = new Image(getClass().getClassLoader().getResource("images/pacRight" + i + ".png").toString());
        }
        ...
        for (int i = 0; i < 2; i++) {
            rightBlinky[i] = new Image(getClass().getClassLoader().getResource("images/blinkyRight" + i + ".png").toString());
        }
    ...
    public void start(Stage stage) throws Exception {
        ...
        Image maze = new Image(getClass().getClassLoader().getResource("images/maze.png").toString());
        Image coin = new Image(getClass().getClassLoader().getResource("images/coin.png").toString());
        ...
        // updates visual display approx 60 times/seconds
        Image title = new Image(getClass().getClassLoader().getResource("images/title.png").toString());
        new AnimationTimer() {
        ...
        pausedMenu.setOnKeyPressed(e -> {
            ...
            if (items.getGameOn() == true) {
                switch (e.getCode()) {
                    ...
                    case S:
                        try {
                            //items.saveToTextFile(getClass().getClassLoader().getResource("savedGame.txt").toString(), avatar, enemy);
                            items.saveToTextFile( (new File( "")).getAbsolutePath() + "/src/main/resources/savedGame.txt", avatar, enemy );     // save to original resource
                            items.saveToTextFile( (new File( "")).getAbsolutePath() + "/build/resources/main/savedGame.txt", avatar, enemy );   // save to resource in the current build
                        } catch (Exception ex) {
                            ex.printStackTrace();
                        }
                        break;
        ...
        stage.sizeToScene(); // gets rid of extra padding around maze image
        stage.show();
    }
